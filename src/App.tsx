...
import { CopyPublisher, DeeplinkPublisher, NativeSharePublisher, XhsSchemePublisher } from "./publishers";
...
export default function App() {
  ...
  async function openReviewPlatformPublish(platform: Extract<SharePlatform, "meituan" | "dianping">) {
    ...
    setCopied(true);
    setPublishMessage(`${platformName}评价文案已复制。正在打开${platformName}，请找到对应商家门店后粘贴发布评价。案例门店：深圳汇职驾校。`);

    window.location.href =
      platform === "meituan"
        ? "http://dpurl.cn/KgRUKrtz"
  ...
  async function oneTapSaveAndPublish() {
    if (!draft) return;
    setPublishMessage("");

    if (sharePlatform === "redbook" && !isAndroid) {
      await nativeShareToRedbook();
      return;
    }
    ...
    setPublishMessage(
      copyResult.success
        ? isAndroid
          ? "文案已复制，图片已开始保存，正在尝试跳转到发布入口。"
          : "文案已复制，图片已开始保存，正在打开发布流程。"
        : copyResult.message || "图片已开始保存。当前浏览器不支持自动复制，请手动复制文案。",
    );
    ...
    <main className="app-shell">
      <div className="phone-frame">
        ...
        <footer className="bottom-cta">
          ...
          {step === "result" && (
            <>
              <div className="publish-cta-tip">文案会自动复制，进入平台后长按粘贴再发布</div>
              <button className="primary-button publish-primary-button" onClick={oneTapSaveAndPublish}>
                <span>
                  <strong>{isAndroid ? "一键保存并跳转发布" : "一键保存并发布"}</strong>
                  <small>{isAndroid ? "保存图片后跳转平台发布入口" : "保存内容到相册，进入平台一键发布"}</small>
                </span>
